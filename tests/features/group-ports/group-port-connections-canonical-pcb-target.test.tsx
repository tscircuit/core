...
test("group connections define the canonical PCB target for an explicit port", async () => {
  const { circuit } = getTestFixture()

  circuit.add(
    <board width="10mm" height="10mm" routingDisabled>
      <group name="G1" showAsSchematicBox connections={{ OUT: "R1.pin1" }}>
        <port name="OUT" direction="left" connectsTo="C1.pin1" />
        <resistor name="R1" resistance="10k" footprint="0402" />
        <capacitor name="C1" capacitance="10uF" footprint="0603" />
      </group>
      <pcbnotetext
        text="OUT follows G1.connections to R1.pin1"
        pcbX={0}
        pcbY={-3}
      />
    </board>,
