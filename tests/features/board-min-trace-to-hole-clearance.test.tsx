...
test("board hole clearance reaches the router and produces fresh routes clear of physical NPTHs", async () => {
  ...
  for (const clearance of [0, 0.2]) {
    ...
    circuit.add(
      <board width={16} height={10} minTraceWidth={0.15}
        minTraceToPadEdgeClearance={0.1} minTraceToHoleClearance={`${clearance}mm`}
        autorouter={{ local: true, algorithmFn: createBasicAutorouter(async (srj) => {
          inputs.push(structuredClone(srj))
          const solver = new AutoroutingPipelineSolver({ ...srj, traces: [] }, { cacheProvider: null })
          solver.solve()
          if (!solver.solved) throw new Error(solver.error ?? "Routing failed")
          return solver.getOutputSimplifiedPcbTraces()
        }) }}>
        {[-5, 5].map((x, index) => (
          <chip key={x} name={`U${index + 1}`} pcbX={x} pinLabels={{ pin1: "SIGNAL" }}
            footprint={<footprint><smtpad portHints={["pin1"]} shape="rect" width={0.8} height={0.8} /></footprint>} />
        ))}
        <pcbnotetext text={`Trace to hole: ${clearance} mm minimum`} pcbY={3} fontSize={0.45} />
    ...
    expect(inputs[0]!.minTraceToHoleClearance).toBe(clearance)
    ...
    expect(inputs[0]!.obstacles.find((o) => o.isHole)).toMatchObject({
      obstacleId: hole.pcb_hole_id, shape: "circle", width: 2, height: 2,
      connectedTo: [], layers: ["top", "bottom"],
    })
    ...
    for (const trace of traces) {
      for (let index = 1; index < trace.route.length; index++) {
        ...
        minimum = Math.min(minimum, pointToSegmentDistance(hole, a, b) - hole.hole_diameter / 2 - Math.max(a.width, b.width) / 2)
      }
    }
