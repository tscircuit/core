...
test("pack-applied rotation propagates to pcb_plated_hole ccw_rotation", async () => {
  ...
  const holeBefore = circuit.db.pcb_plated_hole.list()[0]

  expect(comp.rotation).toBe(0)
  expect(holeBefore.ccw_rotation).toBe(0)

  circuit.source_group_id = circuit.db.source_group.list()[0].source_group_id
  const packOutput = {
    components: [
      {
  ...
  const holeAfter = circuit.db.pcb_plated_hole.list()[0]
