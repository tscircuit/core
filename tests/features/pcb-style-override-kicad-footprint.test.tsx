...
test("kicad footprint loader receives undefined resolvedPcbStyle when not set", async () => {
  let capturedOptions: any = null
  ...
  // Verify that options were passed but resolvedPcbStyle is undefined
  expect(capturedOptions?.resolvedPcbStyle).toBeUndefined()

  const silkscreenTexts = circuit.db.pcb_silkscreen_text.list()

  for (const text of silkscreenTexts) {
    expect(text.font_size).toBe(1.2)
  }
})
