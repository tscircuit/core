...
test("fanout infers plane nets from copper pours", async () => {
  const { circuit } = getTestFixture()
  const autoroutingPhaseIoStack = createAutoroutingPhaseIoStack(circuit)
  ...
  circuit.add(
    ...
    >
      ...
      <chip
        ...
        footprint={
          <footprint>
            <TestPad pinNumber={1} pcbX={0} pcbY={0} />
          </footprint>
        }
      />
      <trace name="GND_A1" from=".U1 > .pin1" to="net.GND" />
  ...
  expect(circuit.db.pcb_autorouting_error.list()).toEqual([])
  expect(autoroutingPhaseIoStack[0]!.startSimpleRouteJson!.buses).toEqual([
    {
      busId: "GND_A1",
      name: "GND_A1",
      connectionNames: [
        autoroutingPhaseIoStack[0]!.startSimpleRouteJson!.connections[0]!.name,
      ],
      termination: { type: "plane", layer: "inner1" },
    },
  ])
