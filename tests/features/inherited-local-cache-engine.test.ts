...
test("a circuit inherits a cache engine installed by its worker host", () => {
  ...
  Reflect.set(globalThis, INHERITED_LOCAL_CACHE_ENGINE_SYMBOL, cacheEngine)

  try {
    const circuit = new RootCircuit()
    expect(circuit.platform?.localCacheEngine).toBe(cacheEngine)
  } finally {
    Reflect.deleteProperty(globalThis, INHERITED_LOCAL_CACHE_ENGINE_SYMBOL)
  }
})
...
test("an explicit platform cache engine takes precedence over the host cache", () => {
  ...
  Reflect.set(globalThis, INHERITED_LOCAL_CACHE_ENGINE_SYMBOL, hostCacheEngine)
