...
test("autoroutingphase algorithmFn routes matching phases and overrides nested callbacks", async () => {
  for (const preset of [undefined, "default"] as const) {
    ...
    circuit.add(
      <board width="18mm" height="12mm">
        ...
        <autoroutingphase
          ...
          autorouter={{
            algorithmFn: async () => {
              throw new Error("The phase-level callback should take precedence")
            },
          }}
          algorithmFn={createPhaseAutorouter()}
        />
    ...
    expect(
      circuit.db.pcb_debug_object
        .list()
        .map((debugObject) => debugObject.label),
    ).toEqual([
      "autorouting phase 0 route-power",
      "autorouting phase 1 route-signal",
    ])
