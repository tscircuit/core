...
test("pcb components include relative positioning metadata 1", async () => {
  ...
  expect(pcbR1.position_mode).toBe("relative_to_group_anchor")
  expect(pcbR1.positioned_relative_to_pcb_group_id).toBe(pcbGroup?.pcb_group_id)
  expect(pcbR1.positioned_relative_to_pcb_board_id).toBeUndefined()

  expect(pcbC1.position_mode).toBe("relative_to_group_anchor")
  expect(pcbC1.positioned_relative_to_pcb_group_id).toBeUndefined()
  expect(pcbC1.positioned_relative_to_pcb_board_id).toBe(pcbBoard.pcb_board_id)
})
