...
test("pcb edge positioning props support calc expressions and are visible in pcb notes", async () => {
  ...
  expect(Number.parseFloat(String(left?.display_offset_x))).toBeCloseTo(
    boardMinX + 2 + (left?.width ?? 0) / 2,
  )
  expect(Number.parseFloat(String(right?.display_offset_x))).toBeCloseTo(
    boardMaxX - 2 - (right?.width ?? 0) / 2,
  )
  expect(Number.parseFloat(String(leftPlus2?.display_offset_x))).toBeCloseTo(
    Number.parseFloat(String(left?.display_offset_x ?? 0)) +
      (left?.width ?? 0) / 2 +
      2 +
      (leftPlus2?.width ?? 0) / 2,
  )
  expect(Number.parseFloat(String(top?.display_offset_y))).toBeCloseTo(
    boardMaxY - 2 - (top?.height ?? 0) / 2,
  )
  expect(Number.parseFloat(String(bottom?.display_offset_y))).toBeCloseTo(
