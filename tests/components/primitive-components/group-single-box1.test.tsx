...
test("should render group as single box with external pins arranged", () => {
  ...
  expect(boxes.length).toBeGreaterThan(0, "No schematic boxes were created")
  const box = boxes[0] // Picking the first box after verifying it exists
  ...
  const ports = circuit.db.schematic_port.list().filter(p => ["D1", "D2"].includes(p.name));
