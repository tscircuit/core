...
test("SchematicSection manual placement: each section has at least one manually positioned component", async () => {
  ...
  circuit.add(
    <board width="60mm" height="50mm" routingDisabled>
      ...
      {/* Power Supply section — U1 manually placed, C1/C2/R1 auto around it */}
      <chip
        name="U1"
        footprint="soic8"
      ...
      {/* Microcontroller section — U2 manually placed, passives auto around it */}
      <chip
        name="U2"
        footprint="soic16"
        schSectionName="mcu"
      ...
      {/* Sensors section — U3 manually placed, C4/R4 auto around it */}
      <chip
        name="U3"
        footprint="soic8"
      ...
      {/* Communication section — U4 manually placed, C5 auto, R5 also manually placed */}
      <chip
        name="U4"
        footprint="soic8"
      ...
      {/* Output section — D1 manually placed, D2/R6/R7 auto around it */}
      <led
        name="D1"
        color="red"
        footprint="led0603"
  ...
  const getSchCenter = (name: string) => {
    const srcId = (
      circuitJson.find(
        (s) => s.type === "source_component" && (s as any).name === name,
      ) as any
    )?.source_component_id
    return (
      schComponents.find((c) => (c as any).source_component_id === srcId) as any
    )?.center
  }

  const u1 = getSchCenter("U1")
  expect(u1.x).toBeCloseTo(-10, 1)
  expect(u1.y).toBeCloseTo(6, 1)

  const u2 = getSchCenter("U2")
  expect(u2.x).toBeCloseTo(0, 1)
  expect(u2.y).toBeCloseTo(6, 1)

  const u3 = getSchCenter("U3")
  expect(u3.x).toBeCloseTo(12, 1)
  expect(u3.y).toBeCloseTo(6, 1)

  const u4 = getSchCenter("U4")
  expect(u4.x).toBeCloseTo(-12, 1)
  expect(u4.y).toBeCloseTo(-6, 1)

  const r5 = getSchCenter("R5")
  expect(r5.x).toBeCloseTo(-10, 1)
  expect(r5.y).toBeCloseTo(-6, 1)

  const d1 = getSchCenter("D1")
