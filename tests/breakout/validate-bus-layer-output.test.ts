...
test("rejects downstream per-net layer reassignment", () => {
  const output: ImplicitBreakoutPointSolverOutput = {
    breakoutPoints: [
      {
        regionId: "left",
        connectionId: "dq0",
      ...
      { regionId: "left", connectionId: "dq1", layer: "inner3", x: -3, y: 0.5 },
      {
        regionId: "right",
        connectionId: "dq0",
        layer: "inner2",
      ...
      { regionId: "right", connectionId: "dq1", layer: "inner3", x: 3, y: 0.5 },
    ],
  }

  expect(() => validateImplicitBreakoutBusOutput(input, output)).toThrow(
    'Implicit breakout bus "byte0" must use one layer, received inner2, inner3',
  )
})
