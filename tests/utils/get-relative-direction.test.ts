...
describe("getRelativeDirection", () => {
  ...
  test("handles collinear displacements where secondary coordinate matches exactly", () => {
    expect(getRelativeDirection({ x: 100, y: 50 }, { x: 500, y: 50 })).toBe("right")
    expect(getRelativeDirection({ x: 500, y: 50 }, { x: 100, y: 50 })).toBe("left")
    expect(getRelativeDirection({ x: 50, y: 100 }, { x: 50, y: 500 })).toBe("up")
    expect(getRelativeDirection({ x: 50, y: 500 }, { x: 50, y: 100 })).toBe("down")
  })
