...
test("applyEditEvents can modify schematic component location", async () => {
  ...
  const updatedSchematicComponent = su(
    updatedCircuitJson,
  ).schematic_component.get(
    circuit.db.schematic_component.list()[0].schematic_component_id,
  )
...
test("applyEditEvents applies multiple edit events in sequence", async () => {
  ...
  const updatedPcbComponent =
    su(updatedCircuitJson).pcb_component.get(pcbComponentId)
  const updatedSchematicComponent =
    su(updatedCircuitJson).schematic_component.get(schematicComponentId)
