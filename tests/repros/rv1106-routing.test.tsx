...
const supplies: Record<string, number[]> = {
  VDD09: [10, 51, 54, 55, 67, 71, 82, 103, 115, 116],
  DDR135: [49, 50, 56],
  VCC18: [53, 70],
  VCC33: [13, 28, 43, 61, 81, 88, 108],
  RTC33: [19],
  USB18: [24],
  CODEC18: [31],
  ANALOG18: [96],
...
function R(p: Passive) {
  return (
    <resistor
      name={p.name}
      resistance={p.value}
      footprint={p.footprint ?? "0402"}
      pcbX={p.x}
      pcbY={p.y}
      layer="top"
      schMarginX={1}
      schMarginY={0.5}
      schSectionName={p.section ?? "Core"}
...
function C(p: Passive) {
  return (
    <capacitor
      ...
      schMarginX={1}
      schMarginY={0.5}
      schSectionName={p.section ?? "Bypass"}
      connections={{ pin1: net(p.a), pin2: net(p.b ?? "GND") }}
    />
  )
}
        ...
        footprint={
          <footprint>
            ...
            <smtpad
              ...
              shape="rect"
            />
            <courtyardrect width={2.8} height={2} />
          </footprint>
        }
        pcbX={x}
        pcbY={y + 3}
...
export default function MinimalRV1106G2() {
  const caps = [
    ...Object.entries(supplies).flatMap(([rail, pins]) =>
      pins.map(() => [rail, "100nF"]),
    ),
    ["VDD09", "10uF"],
    ["VDD09", "10uF"],
  ...
  return (
    ...
    >
      {[
        ...
        <schematicsection name={name} displayName={name} />
      ))}
      {[
        "VDD09",
        "DDR135",
        ...
        <net name={name} routingPhaseIndex={2} />
      ))}
      {["XIN", "XTALOUT", "XOUT"].map((name) => (
        <net name={name} routingPhaseIndex={0} />
      ))}
      {["SCK", "CS", "D0", "D1", "D2", "D3"].map((name) => (
      ...
      />
      {Object.entries(supplies).flatMap(([rail, pins]) =>
        pins.map((p) => <trace key={String(p)} from={pin(p)} to={net(rail)} />),
      )}
      {[37, 57, 129].map((p) => (
        <trace key={String(p)} from={pin(p)} to={net("GND")} />
      ))}
      {caps.map(([rail, value], i) => (
        <C
          key={i}
          name={`CD${i + 1}`}
          value={value}
          a={rail}
          x={i < 28 ? decapPositions[i][0] * 1.55 : -6 + ((i - 28) % 5) * 3}
          y={
            i < 28
              ? 1 + (decapPositions[i][1] - 1) * 1.55
              : 13 + Math.floor((i - 28) / 5) * 2
          }
          section="Bypass"
          footprint={value === "10uF" ? "0603" : "0402"}
        />
      ))}
      {[
        ["VCC33", "RTC33", "100"],
      ...
      />
      <trace from={pin(68)} to={net("XIN")} />
      <trace from={pin(69)} to={net("XOUT")} />
      <R
        name="RXTAL"
        value="22"
        a="XOUT"
      ...
      <crystal
        ...
        pcbX={-13}
        pcbY={0}
        connections={{ pin1: pin(21), pin2: pin(20) }}
      />
      {[21, 20].map((p, i) => (
        <C
          key={String(p)}
      ...
      {[
        ...
        <testpoint
          ...
          padDiameter="1.4mm"
          pcbX={[-23, -23, -23, -23, 23, 23, 23, -23, 23][i]}
          pcbY={[19, 16, -2, -5, 1, 4, -9, -12, 19][i]}
      ...
      <autoroutingphase
        name="remaining"
        phaseIndex={2}
        connections={Array.from({ length: 129 }, (_, i) => pin(i + 1))}
      />
      <autoroutingphase
        name="clocks"
        phaseIndex={0}
...
test("RV1106 exact source board preserves placement and explicit routing phases", async () => {
  ...
  expect(phases.map((phase) => phase.routingPhaseIndex)).toEqual([
    0,
    1,
    2,
    null,
  ...
  expect(
    circuitJson
      ...
      .every((component) => component.layer === "top"),
  ).toBe(true)
