...
test("subcircuit caching benchmark", async () => {
  ...
  // ============================================================
  // Test 3: 20 cached subcircuits nested at different levels
  // Total: 5 direct + 2 in G1 + 3 in G2 + 4 in G3 + 6 in Deep = 20
  // ============================================================
  ...
  // 20 subcircuits × 2 components each = 40 source components
  expect(circuit3.db.source_component.list().length).toBe(40)
  // 20 subcircuits × 2 traces each = 40 traces
  expect(circuit3.db.pcb_trace.list().length).toBe(40)
  // Should only have 1 cached subcircuit (all share the same cache)
  expect(circuit3.cachedSubcircuits!.size).toBe(1)
  ...
  console.log(
    `Speedup factor (nested vs 20x baseline): ${((time1 * 20) / time3).toFixed(2)}x`,
  )
  console.log("=============================================\n")
