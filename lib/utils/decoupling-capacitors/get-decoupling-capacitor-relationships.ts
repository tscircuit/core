...
const sourcePortShouldHaveDecouplingCapacitor = (
  ...
  return getSourcePortLabels(sourcePort).some((label) =>
    INPUT_POWER_PIN_REGEX.test(label),
  )
}

const sourcePortIsGroundFallback = (sourcePort: SourcePort): boolean =>
...
export const getDecouplingCapacitorRelationships = (
  ...
  const getIdsConnectedToSourcePort = (
    ...
    return connectivityNetId
      ? (sourceConnectivityMap.getIdsConnectedToNet(
          connectivityNetId,
  ...
  const sourcePortIsConnectedToGround = (sourcePort: SourcePort): boolean => {
    ...
    if (connectedSourceNets.length > 0) {
      return connectedSourceNets.some(
        (sourceNet) => sourceNet.is_ground === true,
      )
    }
    ...
    return connectedIds
      ...
      .some(sourcePortIsGroundFallback)
  }
...
const relationshipsByRoot = new WeakMap<
  IsolatedCircuit,
  {
    renderCycleId: number
    relationships: DecouplingCapacitorRelationship[]
  }
>()

export const getDecouplingCapacitorRelationshipsForRenderCycle = (
  root: IsolatedCircuit,
): DecouplingCapacitorRelationship[] => {
  const cachedRelationships = relationshipsByRoot.get(root)
  if (cachedRelationships?.renderCycleId === root._renderCycleId) {
    return cachedRelationships.relationships
  }

  const relationships = getDecouplingCapacitorRelationships(root.db)
  relationshipsByRoot.set(root, {
    renderCycleId: root._renderCycleId,
    relationships,
  })
  return relationships
}
