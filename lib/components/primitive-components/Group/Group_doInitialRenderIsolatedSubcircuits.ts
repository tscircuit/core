...
export function Group_doInitialRenderIsolatedSubcircuits(
  ...
  // We clone the children array because we'll clear it on the original group.
  ...
  // Render the isolated circuit - this runs all render phases in isolation
  isolatedCircuit.render()

  // Get all circuit JSON from the isolated render
  ...
  for (const element of isolatedElements) {
    if (element.type === "schematic_group" && element.is_subcircuit === true) {
