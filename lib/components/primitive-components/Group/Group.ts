  ...
  implements ISubcircuit
{
  ...
  doInitialCreateNetsFromProps() {
    // Run MSP routing coordination early, before child components create their traces
    this.doInitialMSPRouting()
  }
  ...
  /**
   * Coordinate MSP (Minimum Spanning Pathfinding) routing at the subcircuit level
   * This detects hub patterns and creates coordinated MSP traces to avoid repetition
   */
  doInitialMSPRouting() {
    if (this.root?.schematicDisabled) return
    if (!this.isSubcircuit) return
    
    console.log(`🏭 ${this.getString()} starting subcircuit-level MSP routing coordination`)
    
    // Detect all hub patterns in this subcircuit
    const hubPatterns = this._detectHubPatterns()
    
    // Create coordinated MSP traces for each hub pattern
    for (const hubPattern of hubPatterns) {
      this._createMSPTracesForHub(hubPattern)
    }
    
    console.log(`🏭 ${this.getString()} completed MSP routing for ${hubPatterns.length} hub patterns`)
  }

  /**
   * Detect hub patterns in this subcircuit where multiple components connect to the same target
   */
  _detectHubPatterns(): Array<{
    hubTarget: string
    ...
    const allComponents = this.selectAll('resistor, capacitor, inductor')
    ...
    console.log(`🔍 ${this.getString()} scanning ${allComponents.length} components for hub patterns...`)
    ...
    for (const [hubTarget, components] of hubMap.entries()) {
      if (components.length >= 2) {
        ...
        const hubPosition = this._getComponentPosition(hubTarget)
   ...
   */
  _createMSPTracesForHub(hubPattern: {
    hubTarget: string
    hubPosition: { x: number, y: number }
    components: Array<{
      name: string
    ...
    // Skip the closest component (it connects directly to the hub)
    ...
    for (let i = 1; i < components.length; i++) {
      ...
      const debugColor = this._getMSPDebugColor(i)
      const debugStyle = this._getMSPDebugStyle(i)
      ...
      const trace = this._createMSPTrace(
        currentComponent,
        previousComponent,
        hubPosition,
        i,
  ...
  /**
   * Create an MSP trace between two components
   */
  _createMSPTrace(
    fromComponent: any,
    toComponent: any,
    ...
    trace._debugLineStyle = debugStyle === 'dashed' ? 'dashed' : 'solid'
   ...
   */
  _getMSPDebugColor(index: number): string {
    const colors = ['#ff0000', '#00ff00', '#0000ff', '#ffff00', '#ff00ff', '#00ffff']
    return colors[index % colors.length]
  }

  /**
   * Get debug style for MSP trace based on chain index  
   */
  _getMSPDebugStyle(index: number): string {
    return 'dashed'
  }

  /**
   * Get component position by selector
   */
  _getComponentPosition(selector: string): { x: number, y: number } | null {
    try {
      const parts = selector.split('.')
      if (parts.length !== 2) return null
      
      const [componentName, pinName] = parts
      const component = this.selectOne(componentName)
      if (!component) {
        // Try different selector formats
        const altComponent = this.selectOne(`.${componentName}`)
        if (altComponent) {
