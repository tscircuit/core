...
export const applyInlineNetLabelEligibility = ({
  ...
  for (const directConnection of directConnections) {
    ...
    const crossSectionConnectionAlreadyOwnsLabel = netConnections.some(
      (netConnection) =>
        netConnection.allowInlineNetLabel &&
        netConnection.isSameSheetCrossSectionConnection &&
        netConnection.connKey === connKey &&
        netConnection.netLabelText === explicitInlineNetLabelText,
    )
    if (crossSectionConnectionAlreadyOwnsLabel) continue
