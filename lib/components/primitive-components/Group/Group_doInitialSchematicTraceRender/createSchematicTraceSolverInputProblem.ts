...
import { dedupeCoincidentInternallyConnectedSchematicPorts } from "./dedupeCoincidentInternallyConnectedSchematicPorts"
...
export function createSchematicTraceSolverInputProblem(
  ...
  for (const [connKey, schematicPortIds] of connKeyToPinIds) {
    ...
    if (sourceNet && schematicPortIds.length >= 1) {
      const uniqueSchematicPortIds =
        dedupeCoincidentInternallyConnectedSchematicPorts({
          db,
          schematicPortIds,
        })
