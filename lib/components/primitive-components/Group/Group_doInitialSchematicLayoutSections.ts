...
>(group: Group<Props>): void {
  ...
  if (hasChildrenWithoutSection) sectionNamesToLayout.push(UNSECTIONED)
  ...
  for (const sectionName of sectionNamesToLayout) {
    ...
    sectionTree.childNodes = sectionTree.childNodes.filter((child) => {
      ...
      return sectionName === UNSECTIONED
        ? compSection === null
        : compSection === sectionName
    })
  ...
  const sectionNameToInfo = new Map<string, SectionBoundsWithChildren>()
    ...
    for (const child of group.children) {
      ...
      const matches =
        sectionName === UNSECTIONED
          ? compSectionName === null
          : compSectionName === sectionName
      if (!matches) continue
