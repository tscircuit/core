...
import { doesElementBelongToBoard } from "lib/utils/doesElementBelongToBoard"
  ...
  implements BoardI, SubcircuitI
{
  ...
  updatePcbDesignRuleChecks() {
    ...
    // Find all board subcircuit IDs
    const boardSubcircuitIds = db.source_board
      .list()
      .map((sb) => {
        const group = db.source_group.get(sb.source_group_id!)
        if (group?.is_subcircuit) return group.subcircuit_id
      })
